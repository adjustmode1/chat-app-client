...
import $ from 'jquery';
import ContentRightGroup from './contentRightGroup';
import Message from './message';
import * as messagesActions from './../../actions/messages';
const DisplayContentMg = (props) => {
    useEffect(()=>{
        $('.content-right').hide();
    ...
    let list = props.messages.map((el,index)=>{
        ...
        if(props.selected.isgroup){
            if(Object.keys(props.selected.member).length>0){
                if(props.selected.member[el.sender]){
    ...
    const send = ()=>{
        ...
        if(mess!=''){
            props.isLogin.socket.emit(emitStr,{'content':mess,'id_group':room});
            $('#sendmessages').val('');
        }
    }
    return (
...
export default connect(mapProps,mapActions)(DisplayContentMg);